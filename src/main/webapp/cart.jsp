...
<script type="text/javascript">
	function pNum(pid,p,no){
		...
		$.ajax({
			url:"car?method=updateCartNum&cid="+pid+"&num=1&price="+p,
			method:"get",
			success:function(){
				location.href = "car?method=getCart";
			},
			error:function(){
				alert("服务器异常");
			}
	...
	function mNum(pid,p,no){
		...
		$.ajax({
			url:"car?method=updateCartNum&cid="+pid+"&num="+num+"&price="+p,
			method:"get",
			success:function(){
				location.href = "car?method=getCart";
			},
	...
	function clearCart(pid){
		if(confirm("确认要删除吗")){
			location.href="car?method=clear&cid="+pid;
		}
	}
</script>
</head>
	 					...
	 					<th width="100px">
		 					<div class="input-group">
		 						<span class="input-group-btn">
		 							<button class="btn btn-default" type="button" onclick="mNum(${c.cid},${c.pprice},${i.count})">-</button>
		 						</span>
		 						<input type="text" class="form-control" id="num_count${i.count}" value="${c.cnum}" readonly="readonly" style="width:40px">
		 						<span class="input-group-btn">
		 							<button class="btn btn-default" type="button" onclick="pNum(${c.cid},${c.pprice},${i.count})">+</button>
		 						</span>
	 						</div>
	 					</th>
	 					<th>¥&nbsp;${c.ccount }</th>
	...
	<div class="row">
		<div class="pull-right" style="margin-right: 40px;">
	            ...
	            <div>
	            	...
	            	&nbsp;&nbsp;
	            	<a href="${pageContext.request.contextPath}/order?method=getOrder" class="btn  btn-danger btn-lg">添加收货地址</a>
