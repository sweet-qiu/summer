	...
	<table cellpadding="0" cellspacing="0" align="center" width="100%" class="table table-striped table-bordered table-hover">
		...
		<tr>
			...
			<td colspan="3">
				<table align="center" cellpadding="0" cellspacing="0" width="100%"  class="table table-striped table-bordered table-hover">
					...
					<c:forEach items="${list}" var="goods" varStatus="i">
						<tr align="center">
							<th>${i.count}</th>
							<th>
								<img src="${pageContext.request.contextPath}/${goods.product.pimage}" width="50px" height="50px">
							</th>
							<th>${goods.product.pname}</th>
							<th>${goods.product.pstate}</th>
							<th>${goods.product.ptime}</th>
							<th>${goods.product.pprice}</th>
							<th>${goods.cnum}</th>
							<th>${goods.ccount}</th>
						</tr>
					</c:forEach>
		...
		<tr>
			<td align="right" colspan="4" style="margin-right: 40px;">
				<a href="${pageContext.request.contextPath }/order?method=showOrder" class="btn btn-danger btn-sm">返回订单列表</a>
				&nbsp;&nbsp;
